...
interface MapData {
  orgName?: string;
  staticArea?: MapNode[];
  fontRemark?: RemarkNode[];
}

async function loadAmrMap() {
  const res = await fetch('/rest/maps/load', {
    method: 'POST',
    headers: {
      authorization: 'mrbase64 mrrest:YWRtaW4mYWRtaW4=',
      token: 'root:eyJhbGciOiJIUzI1NiJ9.eyJjbGllbnRUeXBlIjoiIiwicm9sZUNvZGVzIjoiIiwidXNlckNvZGUiOiJyb290IiwiaWF0IjoxNzc5MTU4NzAwLCJleHAiOjE3NzkyNDUxMDB9.Q2t6AApqqeug3rTzlXCV9Pq6c5_9XmrCvx00s509d2o',
      userName: 'root',
  ...
  return JSON.parse(jsonText) as MapData;
}
...
export default function App() {
  ...
  const nodes: MapNode[] = mapData?.staticArea ?? [];
  const remarks: RemarkNode[] = mapData?.fontRemark ?? [];
  ...
  useEffect(() => {
    ...
    loadAmrMap()
      ...
      .catch((err) => {
        if (cancelled) return;
        console.error('Could not load AMR map.', err);
  ...
  useEffect(() => {
    ...
    draw();
  }, [showLabels, showIds, showGrid, hoveredNode, imagesLoaded, limits, nodes, remarks]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <header>
        <div className="header-logo">
          ...
          <div>
            ...
            <div className="sub">
              AMR Topology Map Viewer &nbsp;·&nbsp; Org: {mapData?.orgName ?? 'Loading...'}
            </div>
          </div>
        </div>
      ...
      <div className="main-layout">
        ...
        >
          ...
          {mapLoading && (
            <div className="hud" style={{ left: '50%', top: '24px', transform: 'translateX(-50%)' }}>
              Loading map...
            </div>
          )}
